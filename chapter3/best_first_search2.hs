...
instance  {-# OVERLAPPING #-} Ord ([City], Int) where
    (<=) (_, x) (_, y) = x <= y
...
findRoute' currentCity targetCity citiesAlreadyVisited totalCost
  ...
  | null availableRoads = ([], 0)
  | otherwise = (currentCity : frontier, cost choosenRoad + frontierCost)
  where
        destinations = sort (map (\road -> (findRoute' (target road) targetCity (currentCity : target road : citiesAlreadyVisited) totalCost, road)) availableRoads)
        destination = head destinations
        choosenRoad = snd destination
        frontier = fst (fst destination)
        frontierCost = snd (fst destination)
        availableRoads
            = filter (\road -> target road `notElem` citiesAlreadyVisited) (roads currentCity)
